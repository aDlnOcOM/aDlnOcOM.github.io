...
function assets(){
  ...
  vm.runInContext(read('resources.js'),scope);
  vm.runInContext(read('game-assets.js'),scope);
  return {A:scope.window.GameAssets,scope,images,timers};
}
test('all 101 assets preload once, report real progress and wait for decoding',async()=>{
  ...
  const second=A.preload();assert.equal(images.length,101);
  images.forEach(image=>image.done());await flush();assert.equal(A.progress().loaded,100);assert.equal(finished,false);
  decode();const result=await pending;await second;
  assert.equal(result.loaded,101);assert.equal(result.failed,0);assert.equal(timers.size,0);
  assert.equal(updates[0],0);assert.equal(updates.at(-1),101);
  assert.ok(updates.every((value,i)=>i===0||value>=updates[i-1]));
  await A.preload();assert.equal(images.length,101,'cache must be reused on repeated calls');
});
test('failed images settle without hanging and retry only requests failed assets',async()=>{
  ...
  const failed=await pending;assert.equal(failed.failed,1);assert.equal(failed.loaded,100);assert.match(failed.failures[0],/player/);
  const retry=A.preload({retry:true});assert.equal(images.length,102);assert.equal(A.progress().loaded,100);
  images[101].done();const result=await retry;assert.equal(result.failed,0);assert.equal(result.loaded,101);assert.equal(A.failures.length,0);
});
test('timeout covers stalled downloads and decode promises; late results cannot corrupt a retry',async()=>{
  ...
  assert.equal((await pending).failed,101);assert.equal(timers.size,0);
  const retry=A.preload({retry:true});images.slice(101).forEach(image=>image.done());await retry;decode();await flush();
  assert.equal(A.progress().loaded,101);assert.equal(A.progress().failed,0);
});
...
test('load works without decode and ignores observer exceptions after subscription',async()=>{
  ...
  images.forEach(image=>{image.decode=undefined;image.done();});assert.equal((await pending).loaded,101);
});
function ui(){
...
test('boot errors stay blocked until retry succeeds; double retry clicks do not duplicate requests',async()=>{
  ...
  get('loading-retry').handlers.click();get('loading-retry').handlers.click();assert.equal(images.length,102);
  get('loading-continue').handlers.click();assert.equal(boot.ready,false,'skip must not bypass an active retry');
  images[101].done();await flush();assert.equal(boot.ready,true);
});
