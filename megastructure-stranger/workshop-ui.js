(() => {
  ...
  let researchQuery='',researchState='all';
  ...
  function render(){
    ...
    const nav=document.createElement('nav');nav.className='workshop-nav workshop-tabs';for(const [id,label]of Object.entries(names)){const b=button(label,()=>{tab=id;if(id==='items'&&category==='resource')category='ranged';page=0;render();});b.setAttribute('aria-pressed',String(tab===id));nav.appendChild(b);}dialog.appendChild(nav);
    ...
    if(tab==='research'){
      ...
      const filters=document.createElement('div');filters.className='workshop-nav workshop-filters';
      const search=document.createElement('input');search.type='search';search.placeholder='Найти исследование…';search.value=researchQuery;search.setAttribute('aria-label','Поиск исследований');
      const status=document.createElement('select');status.setAttribute('aria-label','Статус исследования');
      for(const [id,name] of Object.entries({all:'Все исследования',available:'Доступно сейчас',done:'Изучено',locked:'Недоступно'})){
        const option=document.createElement('option');option.value=id;option.textContent=name;option.selected=id===researchState;status.appendChild(option);
      }
      const count=document.createElement('output');count.setAttribute('aria-live','polite');
      filters.append(search,status,count);dialog.insertBefore(filters,list);
      const cards=[],empty=document.createElement('p');empty.className='empty-state';empty.textContent='Нет исследований по выбранным условиям.';
      for(const r of window.Resources.catalog){const offer=W.researchQuote(p,r.id),done=p.workshop.research[r.id];const el=card(r.name,`${done?'ИССЛЕДОВАНО':'Образцы: '+costs(offer.cost)+' · '+offer.seconds+' с'} · В наличии: ${r.id==='salvage'?p.salvage:p.resources?.[r.id]||0} · Стол ≥ ${offer.level} · Предшественники: ${offer.requires.map(resourceName).join(', ')||'нет'}${!done&&offer.reason?' · '+offer.reason:''}`,r.id);const available=!done&&!offer.reason&&!job&&W.canPay(p,offer.cost);el.dataset.status=done?'done':available?'available':'locked';cards.push({node:el,name:r.name,status:el.dataset.status});el.appendChild(button(done?'Изучено':'Пожертвовать образцы и исследовать',()=>act('research',r.id),done||Boolean(offer.reason)||Boolean(job)||!W.canPay(p,offer.cost)));}
      list.appendChild(empty);
      function filterResearch(){
        let shown=0;for(const item of cards){item.node.hidden=!matchesResearch(item,researchQuery,researchState);if(!item.node.hidden)shown++;}
        count.textContent=`${shown} / ${cards.length}`;empty.hidden=shown>0;
      }
      search.oninput=()=>{researchQuery=search.value;filterResearch();};
      status.onchange=()=>{researchState=status.value;filterResearch();};
      filterResearch();
      return;
    }
    const filters=document.createElement('div');filters.className='workshop-nav workshop-filters';dialog.insertBefore(filters,list);
    ...
    for(const entry of filtered.slice(page*24,page*24+24)){
      ...
      if(def.kind==='ranged')details+=` · ${def.build} · ${def.mass} кг · Эргономика ${def.ergonomics} · Отдача ${Math.round(def.recoil*100)} · Пробитие ${Math.round(def.penetration*100)}% · ${def.modes.map(mode=>mode==='semi'?'ОДИН':'АВТО').join(' / ')}`;
      if(tab==='craft'){
  ...
  function matchesResearch(item,query='',status='all'){
    return item.name.toLocaleLowerCase('ru').includes(query.trim().toLocaleLowerCase('ru'))
      &&(status==='all'||item.status===status);
  }
  window.WorkshopUI={show,matchesResearch};
})();
