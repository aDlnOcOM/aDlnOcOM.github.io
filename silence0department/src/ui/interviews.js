    ...
    <div class="view">
      ...
      <div class="interview-layout">
        ...
        <section class="interview-room">
          ...
          <div class="question-panel"><span>Выберите вопрос. Тон выше повлияет на доверие собеседника.</span><div class="question-grid">
            ${questions.filter(item => !item.locked && !asked.includes(item.id)).map(item => `<button type="button" class="question-button" data-question-id="${item.id}">${escapeHtml(item.question)}</button>`).join("") || '<p class="fine-print">Доступные темы обсуждены. Новые материалы откроют дополнительные вопросы.</p>'}
          </div><details class="locked-questions"><summary>Другие темы · ${questions.filter(item => item.locked || asked.includes(item.id)).length}</summary><div class="question-grid">
            ${questions.filter(item => item.locked || asked.includes(item.id)).map(item => `<button type="button" class="question-button ${asked.includes(item.id) ? "is-asked" : "is-locked"}" data-question-id="${item.id}" disabled>${escapeHtml(item.question)}<small class="question-lock">${asked.includes(item.id) ? "Обсудили · ответ сохранён в разговоре" : escapeHtml(item.lockReason || "Сначала проверьте источник")}</small></button>`).join("")}
          </div></details></div>
        </section>
      </div>
    </div>
