/** Сюжетные профили и правила поля, решений, угрозы и контекстного чата. Без DOM и сети. */
import { PERSON_NAME_BANK } from "../data/catalog.js";
const PROFILES = [
...
// Правило расследования «selectProfile»: работает только с переданным делом и состоянием.
...
// Правило расследования «enrich»: работает только с переданным делом и состоянием.
function enrich(data, profile, rng) {
  ...
  // В категории «маньяк или подражатель» ответ меняется вместе с кодом дела.
  const seriesLinked = profile.id === "serial" || (profile.id === "copycat" && rng() < 0.5);
  const usedNames = new Set([data.victim.name, data.commissioner.name, ...data.suspects.map((person) => person.name), ...data.supportStaff.map((person) => person.name)]);
  const archivedVictim = () => {
    let name;
    do {
      const bank = pick(rng, Object.values(PERSON_NAME_BANK));
      name = `${pick(rng, bank.first)} ${pick(rng, bank.last)}`;
    } while (usedNames.has(name));
    usedNames.add(name);
    return name;
  };
  const episodes = [
    { name: archivedVictim(), daysBefore: 35 + Math.floor(rng() * 15), place: pick(rng, ["служебный корпус вокзала", "ночная мастерская", "закрытая галерея"]) },
    { name: archivedVictim(), daysBefore: 12 + Math.floor(rng() * 10), place: pick(rng, ["склад старого порта", "гостевой дом", "районный клуб"]) },
    { name: data.victim.name, daysBefore: 0, place: data.scenario.location },
  ];
  const titles = {
  ...
  data.mandateText = `Дело касается человека по имени ${data.victim.name}. Проверьте официальную версию и принесите подтверждения, которые можно независимо перепроверить. ${profile.pitch}`;
  data.story = { twist, decoyId: decoy.id, witnessId: witness.id, token, signature, weather, seriesLinked, episodes,
    secondary: seriesLinked || ["stalker", "mercenary"].includes(profile.id),
  ...
  // Одна запись о связи используется обеими сторонами: воспоминания могут различаться, факт контакта — нет.
  data.relationships = [];
  data.suspects.forEach((person, index) => {
    person.life = lives[index % lives.length];
    const next = data.suspects[(index + 1) % data.suspects.length];
    if (!data.relationships.some((edge) => edge.people.includes(person.id) && edge.people.includes(next.id))) {
      data.relationships.push({ people: [person.id, next.id], reason: pick(rng, ["совместная аренда кладовой", "обмен сменами", "заём на ремонт", "знакомство через семью"]) });
    }
  });
  // Городские карточки и беседы читают один граф, не придумывая встречные связи отдельно.
  data.suspects.forEach((person) => {
    const card = data.evidence.find((item) => item.id === `world-${person.id}-links`);
    if (!card) return;
    const contacts = data.relationships.filter((edge) => edge.people.includes(person.id)).map((edge) => {
      const other = data.suspects.find((item) => item.id === edge.people.find((id) => id !== person.id));
      return `${other.name}: ${edge.reason}.`;
    });
    card.content = `Участник: ${person.name}.\nПодтверждённые бытовые связи:\n${contacts.join("\n")}\n\nПривычка: ${person.habit}. Сам по себе контакт не доказывает участие в основном событии.`;
    card.excerpt = "Бытовые связи подтверждаются обеими сторонами; значение контакта требует отдельной проверки.";
  });
  ...
  // Цифровой след проверяют по копии носителя, материальный — с контрольной пробой.
  const digitalTrace = /журнал|датчик|календар|геозон|прошив|подпис|файл|аккаунт|метадан|запис|текст|фрагмент|уведомлен|синхрон|таблиц/i.test(data.trace);
  const traceId = add("field-trace", "Осмотр: след и контрольный источник", `Объект: ${data.incidentAddress}.\nНаблюдение: ${data.trace}.\n${digitalTrace ? "Сохранены исходный носитель и независимая контрольная копия. Экспертная сверка исключает обычную ошибку учёта." : "Сохранены след и отдельная контрольная проба. Экспертная сверка исключает бытовое происхождение."} Подтверждённый механизм — ${data.method}.\nВ журнале выдачи ${digitalTrace ? "носителя" : "упаковки"} указан код ${token}; получателя нужно проверить по оригиналу. След ведёт в место «${address}».`, "field-trace", { type: digitalTrace ? "digital" : "forensics" });
  ...
  if (series) add("field-series", "Сопоставление трёх эпизодов", `Эпизоды:\n${episodes.map((episode) => `${episode.name} · ${episode.daysBefore ? `${episode.daysBefore} дней до текущего события` : "текущее событие"} · ${episode.place}`).join("\n")}\n\n${seriesLinked
    ? `Во всех оригиналах — ${signature}, включая её непубличную внутреннюю сторону. Текущий след соответствует оригиналу, а не ошибочной газетной фотографии. Все три места обслуживал один подрядчик; книга выдачи ${token} и запись бокового входа позволяют проверить нынешнего исполнителя. Связь серии подтверждается, но каждый старый эпизод потребует отдельного доказывания.\nПоследний контакт ещё жив: ${witness.name}. Передайте предупреждение через безопасный канал.`
  ...
  data.scenes = [
    { id: "scene", name: "Место события", subtitle: data.incidentAddress, atmosphere: weather, requires: null, spots: [
      spot("residue", digitalTrace ? "Первичный носитель" : "Повреждённая поверхность", digitalTrace ? "Есть исходная запись и независимая копия. В одной временной отметке заметно расхождение: нужно сопоставление." : "У края заметна чужеродная частица. Простого снимка для сравнения недостаточно.", digitalTrace ? "compare" : "sample", traceId, digitalTrace ? "Первоисточник сопоставлен с контрольной копией. Экспертный вывод и адрес выдачи добавлены в архив." : "Образец и контрольная проба упакованы отдельно. Экспертная сверка и адрес выдачи добавлены в архив."),
  ...
  // Особое дело требует проверки характера эпизода сверх установления личности.
  data.requiredEvidenceIds = data.difficultyKey === "tutorial" ? [] : [motiveId,
    ...(["serial", "copycat", "contract", "mercenary", "stalker"].includes(profile.id) ? ["field-series"] : [])];
...
// Правило расследования «initialState»: работает только с переданным делом и состоянием.
...
// Правило расследования «threat»: работает только с переданным делом и состоянием.
...
// Правило расследования «checkThreat»: работает только с переданным делом и состоянием.
...
// Правило расследования «spend»: работает только с переданным делом и состоянием.
...
// Правило расследования «examine»: работает только с переданным делом и состоянием.
...
// Правило расследования «decide»: работает только с переданным делом и состоянием.
...
// Правило расследования «assistantReply»: работает только с переданным делом и состоянием.
function assistantReply(data, state, text, available) {
  ...
  if (!found.has("field-trace")) {
    const start = data.scenes[0].spots.find((spot) => spot.evidenceId === "field-trace");
    return `Начните с точки «${start.label}» на месте события. ${start.description}`;
  }
...
export default { PROFILES, selectProfile, enrich, initialState, threat, checkThreat, spend, examine, decide, assistantReply };
