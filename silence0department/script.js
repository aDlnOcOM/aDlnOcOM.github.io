(() => {
  ...
  const PERSON_NAME_BANK = {
    male: {
      first: ["Алексей", "Виктор", "Лев", "Михаил", "Роман", "Олег", "Денис", "Артур", "Павел", "Илья", "Глеб", "Борис"],
      last: ["Волков", "Северин", "Крылов", "Ракитин", "Громов", "Соколов", "Рудин", "Туманов", "Чернов", "Яров", "Астахов", "Орлов"],
    },
    female: {
      first: ["Анна", "Марина", "Нина", "Елена", "Вера", "Инга", "Лидия", "Софья", "Тамара", "Кира", "Алиса", "Лариса"],
      last: ["Миронова", "Орлова", "Белова", "Ланская", "Веденеева", "Арсеньева", "Левина", "Зорина", "Мельникова", "Лукина", "Астахова", "Рудина"],
    },
  };
  ...
  function buildMailbox(rng, context) {
    const { victim, culprit, route, incidentMinute, organization, scenario, pattern } = context;
    ...
    return {
      messages: shuffled(rng, [...ordinary, clueMessage, sentMessage]),
      clue,
  ...
  function generateCase(seed, difficultyKey) {
    ...
    const people = uniquePeople(rng, difficulty.suspects + 1);
    ...
    const supplementary = shuffled(rng, puzzlePool.filter((puzzle) => !routePuzzles.includes(puzzle)));
    const uniquePuzzles = (items, limit) => [...new Map(items.filter(Boolean).map((puzzle) => [puzzle.id, puzzle])).values()].slice(0, limit);
    let selectedPuzzles = uniquePuzzles([...routePuzzles, ...supplementary], 3);
    if (difficultyKey === "tutorial") {
      ...
      selectedPuzzles = uniquePuzzles([timelinePuzzle, digitalPuzzle, ...routePuzzles, ...supplementary], 3);
    }
    if (difficultyKey === "detective") selectedPuzzles = uniquePuzzles([...routePuzzles, logicTrapPuzzle, ...supplementary], 5);
    if (difficultyKey === "inspector") selectedPuzzles = uniquePuzzles([...routePuzzles, logicTrapPuzzle, causalPuzzle, ...supplementary], 6);
    const mailbox = buildMailbox(rng, {
      victim: { ...victimPerson, role: victimRole }, culprit, route: discoveryRoute, incidentMinute,
      organization: primaryOrganization, scenario, pattern: crimePattern, alias,
    });
    const evidence = [
      {
        id: "ev-scene",
    ...
    return {
      ...
      evidence: [...evidence, ...worldEvidence, ...conflictEvidence, ...noiseEvidence],
      tasks,
      computer,
      ...
      <div class="view">
        ...
        <div class="archive-tools">
          ...
          <div class="filter-group" role="group" aria-label="Фильтр архива">
            ${[
              ["all", "Все"], ["important", "Важное"], ["conflict", "Конфликты"], ["document", "Документы"], ["log", "Логи"], ["forensics", "Форензика"],
